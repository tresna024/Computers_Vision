...
OUT_DIR   = BASE_DIR / "Random Forest"       # ✅ Ubah target simpan ke visual/Random Forest
OUT_DIR.mkdir(parents=True, exist_ok=True)   # ✅ Otomatis buat folder jika belum ada

# ── Load model ─────────────────────────────────────────────────────────────────
hasil = joblib.load(HASIL_DIR / "hasil_training_rf.pkl")
...
for i in range(len(class_names)):
    ax.add_patch(plt.Rectangle((i, i), 1, 1,
                                fill=False, edgecolor='#2e7d32', lw=2.5, clip_on=False))
...
plt.tight_layout()
plt.savefig(OUT_DIR / "confusion_matrix_rf.png", dpi=150, bbox_inches='tight')
plt.close()
...
ax.set_title('Feature Importance (WITH SMOTE) — Top 20\nRandom Forest', fontsize=14, fontweight='bold', pad=14)
...
plt.tight_layout()
plt.savefig(OUT_DIR / "feature_importance_rf.png", dpi=150, bbox_inches='tight')
plt.close()
print("Feature importance disimpan ke: Random Forest/feature_importance_rf.png")

# ══════════════════════════════════════════════════════════════════════════════
# 3. PCA 3D — Penyebaran Data Ekstraksi Uji (WITH SMOTE)
# ══════════════════════════════════════════════════════════════════════════════
...
plt.tight_layout()
plt.savefig(OUT_DIR / "pca_3d_scatter_rf.png", dpi=150, bbox_inches='tight')
plt.close()
